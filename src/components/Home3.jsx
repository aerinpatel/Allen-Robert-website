...
const VideoCard = ({ title, views, time, youtubeId }) => {
  const [isPlaying, setIsPlaying] = useState(false);

  // Added controls=0 to hide bottom bar
    ...
    >
      <div className="absolute inset-0 w-full h-full overflow-hidden pointer-events-none">
        <iframe
          ...
          /* CHANGES MADE HERE:
             1. We use h-[120%] and w-[250%] to make the video larger than the container.
             2. We use -translate-y-[52%] to push the top title bar out of view.
          */
          className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-[52%] h-[120%] w-[250%] max-w-none object-cover scale-105"
          frameBorder="0"
      ...
      {/* Overlays - Only visible when NOT playing */}
      {!isPlaying && (
        <>
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4 pt-12 pb-14 
          ...
          >
            ...
            <div className="flex items-center text-xs text-white/90 font-medium">
              ...
              <span className="flex items-center">
                <Clock className="w-3 h-3 mr-1" /> {time || "Video"}
              </span>
            </div>
          </div>
...
const StoryVideoPlayer = () => {
  ...
  return (
    ...
    >
      {/* CHANGES MADE HERE:
          We wrap the iframe in a container that is slightly smaller than the video itself 
          to crop the top (title) and bottom (progress bar) 
      */}
